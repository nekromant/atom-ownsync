...
export default class AtomOwnSyncCore {
    ...
    isTarballFresh() {
        ...
        console.log(archive)
        stats = fs.stat(archive, this.gotArchiveStat);
    }

    gotArchiveStat(archiveStat)
    {
        console.log("Got " + archiveStat)
        path = atom.getLoadSettings().atomHome;
        dirObj = new Directory(path);
        this.archiveStat = archiveStat;
        /* Man, I hate js callback hell */
        handler = this.gotDirContents
        ent = dirObj.getEntries(this.gotDirContents)

    }

    gotDirContents(err, items)
    {
        path = atom.getLoadSettings().atomHome;
        handler = this.gotFileStat
        items.forEach(function(item, i, arr) {
            el = path + '/' + item.getBaseName();
            /* Fire a shitload stat()s, get results in callbacks */
            stats = fs.stat(el, this.gotFileStat)
        })
    }

    gotFileStat(err, stats)
    {
        if (!this.operationInProgress)
            return; /* We've already decided */
        if (archiveStat.mtime < stats.mtime) {
            console.log("AtomOwnSync: object " + name + " modified, archive invalid")
        }
    }


    showTarballInfo() {
        console.log('AtomOwnSync: Reading tarball info!' + this.isTarballFresh());
